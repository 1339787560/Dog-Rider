...
**实测事实：** 完全相同的上下文、完全相同的 prompt，连续运行 10-20 次：

- **~85% 的请求**：1 轮结束，命中率稳定在 **97.8%**（模型直接输出回答）
- **~15% 的请求**：模型选择调用工具（read_file/grep），产生 2-4 轮交互，第二轮起命中率降至 67-90%

**原因：** LLM 的非确定性（temperature > 0）。对于边界模糊的问题（如"介绍一下你自己"），模型在"直接回答"和"先读文件确认"两个路径上随机采样。

**对缓存命中率的影响：** 多轮测试报告中出现的低命中率样本，不是缓存失效，而是模型选择了工具调用路径——该路径的上下文组合（assistant tool call + tool result）是唯一的，天然无法命中缓存。
